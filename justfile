# Quality gates for Bitty File Manager (bitty-terminal.file-manager).
#
# Dependencies are pinned in package.json and locked in bun.lock; `just
# install` materializes them and `just deps` fails closed when they are
# missing. Every gate below runs offline once install has completed; never use
...
lua: deps
    bun run luaparse --quiet --file lua/file-manager/init.lua
    bun run luaparse --quiet --file lua/file-manager/scope.lua
    bun run luaparse --quiet --file lua/file-manager/listing.lua
    bun run luaparse --quiet --file lua/file-manager/scene.lua

# Run the Lua 5.4 behavior suite (scope, listings, scenes,
...
# authoritative SDK linter (`base.toml` is the accepted positive control).
# Fails when the fixture directory, a required fixture, or the pinned linter
# is missing.
test-negative: deps
    bun tests/check-manifest-negative.mjs

# Run all behavior and conformance tests.
test: test-lua test-luals test-negative
...
# (`cd "$BITTY_WORKSPACE/file-manager" && just workflow-publish`).
