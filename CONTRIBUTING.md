...
This guide is for contributors to the `file-manager` plugin repository. The
plugin is observation-only and pre-release: listing, navigation, and preview
policy plus their behavior suite exist, but no release has shipped and
...
plugin requests only `terminal.semantic-read` (read-only cwd/title
observation); any wider request requires an explicitly scoped task plus a
reviewed privacy and security note. Never add high-risk capabilities, install
scripts, secrets, or ambient authority as a side effect of an unrelated
change.
